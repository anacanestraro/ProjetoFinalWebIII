...
<style>
    ...
    .avatar { width: 2rem; height: 2rem; border-radius: 50%; background: #ede9fe; display: inline-flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 600; color: #7c3aed; }
...
<div class="list-page">
    <div class="list-wrap">
        <div class="list-header">
            <h1 class="list-title">Clientes</h1>
            <a href="{{ route('cliente.create') }}" class="btn-new">
        ...
        <div class="panel">
            <div style="padding: 0.875rem 1rem; border-bottom: 1px solid #e5e7eb;">
                <input class="search-box" oninput="filtrar(this.value)" type="text" placeholder="Buscar cliente...">
            </div>
            <table class="list-table">
                ...
                <tbody>
                    ...
                    <tr class="cliente-row">
                        ...
                        <td><span class="item-name nome">{{ $cliente->nome }}</span></td>
                        <td>{{ $cliente->email ?? '—' }}</td>
                        <td>{{ $cliente->telefone ?? '—' }}</td>
                        <td style="text-align:right; white-space:nowrap;">
                            <a href="{{ route('cliente.show', $cliente->id) }}" class="action-link">Ver</a>
                            <a href="{{ route('cliente.edit', $cliente->id) }}" class="action-link">Editar</a>
                            <form action="{{ route('cliente.destroy', $cliente->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Tem certeza que deseja deletar?');">
                                @csrf
                                @method('DELETE')
