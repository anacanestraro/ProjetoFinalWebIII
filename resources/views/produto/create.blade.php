...
<style>
    ...
    .img-preview-wrap { border: 2px dashed #e5e7eb; border-radius: 0.5rem; background: #f9fafb; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 1.5rem; gap: 0.75rem; cursor: pointer; transition: border-color .15s; }
    .img-preview-wrap:hover { border-color: #9ca3af; }
    .img-preview { max-height: 140px; max-width: 100%; object-fit: contain; display: none; border-radius: 0.375rem; }
    .img-placeholder { display: flex; flex-direction: column; align-items: center; gap: 0.5rem; }
    .img-placeholder svg { width: 2rem; height: 2rem; stroke: #9ca3af; }
    .img-placeholder span { font-size: 0.8rem; color: #9ca3af; }
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .img-preview-wrap:hover { border-color: #6b7280; }
            ...
            <form action="{{ route('produto.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="panel-body">
                    ...
                    <div>
                        ...
                        <div class="img-preview-wrap" onclick="document.getElementById('imagem').click()">
                        ...
                        <input type="file" id="imagem" name="imagem" class="file-input-hidden" accept="image/*" required onchange="previewImagem(event)">
                    ...
                    <div>
                        ...
                        <input class="field-input" type="text" name="descricao" value="{{ old('descricao') }}">
                    ...
                    <div class="field-grid">
                        <div>
                            ...
                            <input class="field-input" type="text" name="valorUnitario" placeholder="0,00" value="{{ old('valorUnitario') }}">
                            @error('valorUnitario') <p class="field-error">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="field-label">Estoque</label>
                            <input class="field-input" type="number" name="estoque" min="0" value="{{ old('estoque', 0) }}">
                    ...
                    <div class="field-grid">
                        <div>
                            ...
                            <select class="field-select" name="id_unidade" required>
                                <option value="">Selecione...</option>
                                @foreach($unidades as $unidade)
                                    <option value="{{ $unidade->id }}" {{ old('id_unidade') == $unidade->id ? 'selected' : '' }}>
                                        {{ $unidade->sigla }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select class="field-select" name="id_categoria" required>
                                <option value="">Selecione...</option>
                ...
                <div class="panel-footer">
                    ...
                    <a href="{{ route('produto.index') }}" class="btn-cancel">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
