...
<div class="form-page">
    <div class="form-wrap">
        <div class="form-header">
            <h1 class="form-title">Editar unidade</h1>
            <a href="{{ route('unidade.index') }}" class="form-back">
            ...
            <form action="{{ route('unidade.update', $unidade) }}" method="POST">
                ...
                <div class="panel-body">
                    <div>
                        ...
                        <input class="field-input" type="text" name="sigla" value="{{ old('sigla', $unidade->sigla) }}">
                        @error('sigla') <p class="field-error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="field-label">Descrição</label>
                        <input class="field-input" type="text" name="descricao" value="{{ old('descricao', $unidade->descricao) }}">
                        @error('descricao') <p class="field-error">{{ $message }}</p> @enderror
                    </div>
                </div>
                <div class="panel-footer">
                    <button type="submit" class="btn-save">Salvar alterações</button>
                    <a href="{{ route('unidade.show', $unidade->id) }}" class="btn-cancel">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
