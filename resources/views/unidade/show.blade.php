...
<style>
    ...
    .sigla-big { width: 3rem; height: 3rem; border-radius: 0.5rem; background: #f3f4f6; display: flex; align-items: center; justify-content: center; font-size: 1rem; font-weight: 700; color: #374151; flex-shrink: 0; }
    .panel-name { font-size: 1rem; font-weight: 500; color: #111827; margin: 0; }
    .panel-sub { font-size: 0.8rem; color: #9ca3af; margin: 0.125rem 0 0; }
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .sigla-big { background: #374151; color: #d1d5db; }
...
<div class="show-page">
    <div class="show-wrap">
        <div class="show-header">
            <h1 class="show-title">Detalhes da unidade</h1>
            <a href="{{ route('unidade.index') }}" class="show-back">
        ...
        <div class="panel">
            <div class="panel-top">
                <div class="sigla-big">{{ $unidade->sigla }}</div>
                <div>
                    <p class="panel-name">{{ $unidade->sigla }}</p>
                    <p class="panel-sub">{{ $unidade->descricao ?? 'Sem descrição' }}</p>
                </div>
            </div>
            <div class="section-label">Informações</div>
            <div class="panel-body">
                <div class="field-row">
                    <span class="field-label">Sigla</span>
                    <span class="field-value">{{ $unidade->sigla }}</span>
                </div>
                <div class="field-row">
                    <span class="field-label">Descrição</span>
                    <span class="field-value">{{ $unidade->descricao ?? '—' }}</span>
                </div>
            </div>
            <div class="actions">
                <a href="{{ route('unidade.edit', $unidade->id) }}" class="btn-edit">Editar</a>
                <form action="{{ route('unidade.destroy', $unidade->id) }}" method="POST" style="flex:1;" onsubmit="return confirm('Tem certeza que deseja deletar?');">
                    @csrf
                    @method('DELETE')
