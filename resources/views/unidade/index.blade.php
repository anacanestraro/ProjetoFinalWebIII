...
<style>
    ...
    .sigla-badge { display: inline-block; font-size: 0.75rem; font-weight: 600; padding: 0.2rem 0.6rem; border-radius: 0.375rem; background: #f3f4f6; color: #374151; }
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .sigla-badge { background: #374151; color: #d1d5db; }
...
<div class="list-page">
    <div class="list-wrap">
        <div class="list-header">
            <h1 class="list-title">Unidades</h1>
            <a href="{{ route('unidade.create') }}" class="btn-new">
        ...
        <div class="panel">
            <div style="padding: 0.875rem 1rem; border-bottom: 1px solid #e5e7eb;">
                <input class="search-box" oninput="filtrar(this.value)" type="text" placeholder="Buscar unidade...">
            </div>
            <table class="list-table">
                ...
                <tbody>
                    ...
                    <tr class="unidade-row">
                        <td><span class="sigla-badge nome">{{ $unidade->sigla }}</span></td>
                        <td><span class="item-name">{{ $unidade->descricao ?? '—' }}</span></td>
                        <td style="text-align:right; white-space:nowrap;">
                            <a href="{{ route('unidade.show', $unidade->id) }}" class="action-link">Ver</a>
                            <a href="{{ route('unidade.edit', $unidade->id) }}" class="action-link">Editar</a>
                            <form action="{{ route('unidade.destroy', $unidade->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Tem certeza que deseja deletar?');">
                                @csrf
                                @method('DELETE')
