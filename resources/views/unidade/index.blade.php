    ...
    <div class="flex items-center justify-center h-screen mt-4">
        <div class="w-11/12 sm:w-11/12 md:w-12 lg:w-6/12 p-6 rounded-lg border-gray-100 border">
            <div class="w-full flex justify-between items-center p-3 gap-6">
                <h2 class="text-xl font-semibold dark:text-gray-100">Unidades</h2>
                <button class="flex items-center bg-gradient-to-r from-violet-300 to-indigo-300 border border-fuchsia-00 hover:border-violet-100 text-white font-semibold py-2 px-4 rounded-md transition-colors duration-300">
                    ...
                    <a href="{{ route('unidade.create') }}" class="text-white">Nova unidade</a>
                </button>
            </div>
            <div class="w-full flex justify-center py-4 mb-4 relative w-full">
                <x-text-input class="w-full" oninput="filtrarUnidade(this.value)" type="text" placeholder="Buscar..." />
            </div>
            <!-- Cards unidades cadastrados -->
            <p class="mb-4 text-x2 font-semibold dark:text-gray-100">{{session('mensagem')}}</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-4">
                @foreach($unidades as $unidade)
                <div class="unidade w-full max-w-sm border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
                    <div class=" flex flex-col items-center pb-10">
                        <h5 class="nome mb-1 text-xl font-medium text-gray-900 dark:text-white">{{$unidade->sigla}}</h5>
                        <span class="text-sm text-gray-500 dark:text-gray400 px-2">{{$unidade->descricao}}</span>
                        <div class="flex mt-4 md:mt-6">
                            <a href="{{ route('unidade.edit', $unidade->id) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white "> Editar</a>
                        ...
                        <a href="{{ route('unidade.show', $unidade->id) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white "> Visualizar detalhes</a>
                    </div>
...
<script>
    function filtrarUnidade(valor) { // recebendo o valor que é colocado no input
        const linhas = document.querySelectorAll(".unidade"); // seleciona as linhas da tabela que possuem a classe unidade
